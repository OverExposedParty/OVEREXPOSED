(function initialiseOEAudio() {
  ...
  const INTERACTION_SOUND_RULES = [
    {
      ...
      resolveIntent: (control) =>
        isControlSelected(control) ? 'deselect' : 'select'
    },
    {
  ...
  function normaliseDefinition(key, definition) {
    const value =
      typeof definition === 'string' ? { src: definition } : definition;
    ...
    return {
      ...
      playbackRateVariation: Math.max(
        0,
        Number(value.playbackRateVariation) || 0
      ),
      lane:
        value.lane === 'independent'
          ? 'independent'
  ...
  function applyAudioOptions(audio, definition, options = {}) {
    ...
    const baseRate = Math.max(
      0.01,
      Number(options.playbackRate ?? definition.playbackRate) || 1
    );
    const variation = Math.max(
      0,
      Number(
        options.playbackRateVariation ?? definition.playbackRateVariation
      ) || 0
    );
  ...
  function prepareAudioElement(audio, definition, options = {}) {
    ...
    try {
      ...
      emitAudioDebug(
        'warn',
        'audio.errors',
        'Audio position could not be reset.',
        {
  ...
  function unlockAudio() {
    ...
    emitAudioDebug(
      'debug',
      'audio.playback',
      'Attempting browser audio unlock.',
      {
  ...
  function load(key) {
    ...
    return new Promise((resolve) => {
      ...
      audio.addEventListener('canplaythrough', () => finish('canplaythrough'), {
        once: true
      });
      audio.addEventListener('loadeddata', () => finish('loadeddata'), {
        once: true
      });
  ...
  async function playAudio(key, options = {}) {
    ...
    const cooldown = Math.max(
      0,
      Number(options.cooldown ?? definition.cooldown) || 0
    );
    ...
    const maxInstances = Math.max(
      1,
      Number(options.maxInstances ?? definition.maxInstances) || Infinity
    );
  ...
  async function runSequenceLaneRequest(request) {
    ...
    for (const [itemIndex, item] of request.items.entries()) {
      ...
      if (
        request.cancelled ||
        activeLaneRequests.get(request.lane) !== request
      ) {
        break;
      }
  ...
  function startLaneRequest(request) {
    ...
    Promise.resolve(runner).catch((error) => {
      ...
      if (!request.playback) {
        ...
        logRemainingSequenceItemsSkipped(request, 'lane_request_failed');
      }
      settleLaneRequest(request, null);
      finishLaneRequest(request);
    });
  ...
  function playSequence(keys, options = {}) {
    ...
    const playback = getPlaybackConfiguration(firstDefinition, {
      ...
      priority:
        options.priority ??
        Math.max(...items.map((item) => item.definition.priority)),
  ...
  function stopLane(lane = DEFAULT_LANE, options = {}) {
    ...
    const hadQueuedRequest =
      options.clearQueue === false
        ? false
  ...
  function setEnabled(nextEnabled, options = {}) {
    ...
    window.dispatchEvent(
      new CustomEvent('oe:audio-settings-changed', {
  ...
  register({
    containerOpen: {
      src: '/sounds/ui/containers/open.wav',
      group: 'ui',
      preload: true
    },
    containerClose: {
      src: '/sounds/ui/containers/close.wav',
      group: 'ui',
      preload: true
    },
    uiToggleEnabled: {
      src: '/sounds/ui/toggles/enabled.wav',
      group: 'ui',
      preload: true
    },
    uiToggleDisabled: {
      src: '/sounds/ui/toggles/disabled.wav',
      group: 'ui',
      preload: true
    },
    splashScreenUp: {
      src: '/sounds/ui/splash-screen/up.wav',
      group: 'ui',
      preload: true
    },
    splashScreenDown: {
      src: '/sounds/ui/splash-screen/down.wav',
      group: 'ui',
      preload: true
    },
    sidePanelOpen: {
      src: '/sounds/olings/lab/side-panels/side-panel-open.wav',
      group: 'ui',
      preload: true
    },
    sidePanelClose: {
      src: '/sounds/olings/lab/side-panels/side-panel-close.wav',
      group: 'ui',
      preload: true
    },
    uiSelect: {
    ...
    uiConfirm: {
      src: '/sounds/ui/buttons/actions/confirm.wav',
      group: 'ui',
      preload: true,
      cooldown: 80
    },
    uiSuccess: {
      src: '/sounds/ui/buttons/actions/success.wav',
      group: 'ui',
      preload: true,
      cooldown: 80
    },
    ...
    uiDragPickup: {
      src: '/sounds/ui/drag-and-drop/pickup.wav',
      group: 'ui',
      preload: true,
      cooldown: 60,
      lane: 'independent',
    ...
    uiDragMove: {
      src: '/sounds/ui/drag-and-drop/move.wav',
      group: 'ui',
      preload: true,
      cooldown: 60,
      maxInstances: 1
    ...
    uiDragPlace: {
      src: '/sounds/ui/drag-and-drop/place.wav',
      group: 'ui',
      preload: true,
      cooldown: 60,
      maxInstances: 1
    ...
    uiDragStore: {
      src: '/sounds/ui/drag-and-drop/store.wav',
      group: 'ui',
      preload: true,
      cooldown: 60,
      maxInstances: 1
    ...
    economyPurchase: {
      src: '/sounds/economy/purchase.wav',
      group: 'ui',
      preload: true,
      cooldown: 120,
      maxInstances: 1,
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      if (requestedSound === 'none' || control.classList.contains('sound-save'))
        return;
      ...
      const rule = INTERACTION_SOUND_RULES.find(({ selector }) =>
        control.matches(selector)
      );
      if (!rule) return;

      const intent = rule.resolveIntent
        ? rule.resolveIntent(control)
        : rule.intent;
