...
function ensureOverexposureConsoleCommands() {
    if (!overexposureConsoleCommandsReady) {
        overexposureConsoleCommandsReady = loadOverexposureConsoleScript('/scripts/general/commands/command-registry.js')
            .then(() => Promise.all([
                loadOverexposureConsoleScript('/scripts/general/commands/achievement-commands.js'),
                loadOverexposureConsoleScript('/scripts/general/commands/oling-commands.js'),
                loadOverexposureConsoleScript('/scripts/general/commands/shop-commands.js'),
                loadOverexposureConsoleScript('/scripts/party-games/general/party-games-commands.js'),
                loadOverexposureConsoleScript('/scripts/overexposure/commands/overexposure-commands.js')
            ]))
            .then(async () => {
                overexposureConsoleCommandSuggestions = await getOverexposureConsoleCommandSuggestions();
        ...
        overexposureConsoleAchievementKeysReady = fetch('/api/achievements')
            ...
            .then((data) =>
                Array.isArray(data?.data?.achievements)
                    ? data.data.achievements.map((achievement) => achievement.key).filter(Boolean)
                    : Array.isArray(data?.achievements)
                        ? data.achievements.map((achievement) => achievement.key).filter(Boolean)
                        : []
            )
            .catch(() => []);
    }
...
function getSuggestionsFromCommandState(pageType) {
    ...
    ['global', pageType]
        .filter((packId, index, packIds) => packId && packIds.indexOf(packId) === index)
        .forEach((packId) => {
...
async function getOverexposureConsoleCommandSuggestions() {
    const asyncRegistrySuggestions = await window.OverexposedCommands?.getCommandSuggestionsAsync?.('overexposure');
    const registrySuggestions = window.OverexposedCommands?.getCommandSuggestions?.('overexposure');
    ...
    const achievementSuggestions = achievementKeys.map((key) => `/notification test achievement ${key}`);
