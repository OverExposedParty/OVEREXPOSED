(function () {
  function createOePanelActionFormFields(options) {
    ...
    function createPackFormField(labelText, name, options = {}) {
      ...
      input.name = name;
      if (!options.options && !options.multiline) input.type = 'text';
      ...
      if (options.rows && options.multiline) input.rows = options.rows;
      ...
      const control = options.paletteInput
        ? window.OE_PANEL_PALETTES?.createColourInput(input) || input
        : input;
