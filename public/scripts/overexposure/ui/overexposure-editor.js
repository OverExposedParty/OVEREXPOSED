...
function SetOverexposureClassArray() {
    if (isOverexposureEditorEmpty()) {
        addElementIfNotExists(elementClassArray, overexposureContainer, { sound: false });
        removeElementIfExists(permanantElementClassArray, overexposureContainer, { sound: false });
    }
    else {
        addElementIfNotExists(permanantElementClassArray, overexposureContainer, { sound: false });
        removeElementIfExists(elementClassArray, overexposureContainer, { sound: false });
    }
}
...
function ToggleOverexposureContainer({ toggle = false, button = null, draft = false, force = false } = {}) {
    ...
    if (toggle == true && permanantElementClassArray.includes(overexposureContainer) == false) {
        ...
        const openedDeferredOeCustomisation =
            typeof window.openDeferredOverexposureOeCustomisation === 'function' &&
            window.openDeferredOverexposureOeCustomisation() === true;

        if (!openedDeferredOeCustomisation && typeof syncOverlayStack === 'function') {
            syncOverlayStack();
        }
    }
    if (isContainerVisible(overexposureContainer) && toggle == true) {
        ...
        prepareOverexposureDeletePostContainer(dataId);
        prepareOverexposureReportPostContainer(dataId, postTitle);
