...
const { exportGamePacksToJson } = require('../server/services/game-packs');
...
async function main() {
  ...
  console.log(`Exported ${exported.length} game packs to local JSON files.`);
}

main()
  .catch((error) => {
    console.error('Failed to export game packs:', error);
    process.exitCode = 1;
  })
  .finally(async () => {
