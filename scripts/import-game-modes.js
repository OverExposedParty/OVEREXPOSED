...
const { importGameModesFromJson } = require('../server/services/game-modes');
...
async function main() {
  ...
  const imported = await importGameModesFromJson(GameMode);
  console.log(`Imported ${imported.length} game modes into MongoDB.`);
}
