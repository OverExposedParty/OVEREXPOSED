...
const { exportGameRulesToJson } = require('../server/services/game-rules');
...
async function main() {
  ...
  console.log(`Exported ${exported.length} game rules to local JSON files.`);
}

main()
  .catch((error) => {
    console.error('Failed to export game rules:', error);
    process.exitCode = 1;
  })
  .finally(async () => {
