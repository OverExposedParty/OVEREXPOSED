...
test('assertPartyActionBody accepts the host return-to-lobby action', () => {
  assert.doesNotThrow(() => {
    assertPartyActionBody({
      partyId: 'ABC-123',
...
test('assertPartyUpdateBody accepts session metadata updates', () => {
  assert.doesNotThrow(() => {
    assertPartyUpdateBody(
      {
        partyId: 'ABC-123',
        session: {
          gameId: 'TOD-A1B2C3D4',
          createdAt: '2026-05-20T14:35:22.123Z',
          serverRegion: 'LON1'
        }
      },
      ['session', 'config', 'state', 'players']
    );
  });
});
...
test('assertRemovePlayerBody accepts only the supported main menu exit intent', () => {
  assert.doesNotThrow(() =>
    assertRemovePlayerBody({
      partyId: 'ABC-123',
      computerIdToRemove: 'device-1',
      actorComputerId: 'device-1',
...
test('Mafia start requests reject client role assignments', () => {
  assert.throws(
    () =>
      assertPartyActionBody({
        partyId: 'ABC-123',
        ...
        actorId: 'host-1',
        payload: {
          shuffledRoles: ['mafioso', 'civilian']
        }
      }),
    /shuffledRoles is not allowed/
  );
  assert.throws(
    () =>
      assertPartyActionBody({
        ...
        actorId: 'host-1',
        payload: {
          assignedRoleKeys: ['mafioso', 'civilian']
        }
      }),
    /assignedRoleKeys is not allowed/
  );
});

test('Mafia start requests no longer accept role assignment responsibility', () => {
  assert.doesNotThrow(() =>
    assertPartyActionBody({
      partyId: 'ABC-123',
      action: 'mafia-start-game',
